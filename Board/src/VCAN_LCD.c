...
void LCD_numf(Site_t site, double num, uint16 Color, uint16 bkColor) //这个是自己写的，用来显示两位小数的浮点型
{
    ...
    site.x += LCD_num(site, (uint32)(0.005+num), Color, bkColor)*8;
    num += 0.005; 
    num*=10;
  ...
  /*
	...
	if (0 == flag1 && 0 == flag2)
	{
		...
		while (temp2)
		{
			...
			LCD_char(sitetemp, (temp2 % 10) + '0', Color, bkColor);
			if (t == (s - 2))
			{
				sitetemp.x = site.x + (8 * (t--));
	...
	else if (0 == flag1 && flag2 != 0)
	{
		for (i = 0; i < t; i++)
		{
			sitetemp.x = site.x + (8 * (3-i));
			LCD_char(sitetemp, (temp2 % 10) + '0', Color, bkColor);
			temp2 /= 10;
		}
		while (flag2)
		{
			if (1 == flag2)
			{
				...
				LCD_char(sitetemp, '.', Color, bkColor);
				sitetemp.x = site.x;
				LCD_char(sitetemp, '0', Color, bkColor);
				break;
			}
			sitetemp.x = site.x + (8 * (flag2));
			LCD_char(sitetemp, '0', Color, bkColor);
			flag2--;
		}
	}
	else if (flag1 != 0 && 0 == flag2)
	{
		t++;
		s = t;
		while (temp2)
	...
	else
	{
		...
		while (flag2)
		{
			if (1 == flag2)
			{
				sitetemp.x = site.x + (8*2);
				LCD_char(sitetemp, '.', Color, bkColor);
				sitetemp.x = site.x + 8;
				LCD_char(sitetemp, '0', Color, bkColor);
				break;
			}
			sitetemp.x = site.x + (8 * (flag2 + 1));
			LCD_char(sitetemp, '0', Color, bkColor);
			flag2--;
		}
		sitetemp.x = site.x;
