...
volatile IMG_STATUS_e      ov7725_eagle_img_flag = IMG_FINISH;   //图像状态，此时为图像采集完毕
...
{
    //寄存器，寄存器值次
    {OV7725_COM4         , 0xC1}, //PLL 8X,ACE FULLWINDOW
    {OV7725_CLKRC        , 0x00}, //分频，(input clock)/=((OV7725_CLKRC)+1)*2
    {OV7725_COM2         , 0x03}, //4x output drive capability,
    {OV7725_COM3         , 0xD0}, //水平垂直翻转，y/uv格式输出序列交换但其为输出模式
    {OV7725_COM7         , 0x40}, //GBR4:2:2,y/uv,qvga(分辨率)
    {OV7725_COM10        , 0x20},
    {OV7725_HSTART       , 0x3F},
    {OV7725_HSIZE        , 0x50},
    {OV7725_VSTRT        , 0x03},
